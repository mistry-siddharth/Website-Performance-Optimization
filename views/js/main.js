...
var resizePizzas = function(size) {
  ...
  /* determineDx function is not needed to switch the sizes of pizzas. Extracted just sizeSwitcher to do the work */
  ...
  var windWdth = document.getElementById("randomPizzas").offsetWidth;
  /* Iterates through pizza elements on the page and changes their width.
  Replaced querySelector with getElementsByClassName as it is faster. */
  function changePizzaSizes(size) {
    var randomPizzas = document.getElementsByClassName("randomPizzaContainer");
    for (var i = 0; i < randomPizzas.length; i++) {
      randomPizzas[i].style.width = (sizeSwitcher(size) * windWdth) + 'px';
    }
  }
...
/* 'items' is declared on the top in the global scope, and then assigned a value only once, on page load. */
...
function updatePositions() {
  ...
  var phases = [];
  for (var i = 0; i < 5; i++) {
    /* moved the Math.sin out of the loop and placed it in this loop so that it
    doesn't get called repeatedly. And store each calculated phase in phases array*/
    phases.push(Math.sin(phase + i));
  }
...
document.addEventListener('DOMContentLoaded', function() {
  /* Instead of running the loop 200 times, now it'll loop only till the screen height and will change dynamically.
  Replaced querySelector with getElementById as it is faster. */
  var cols = 8;
  var s = 256;
  var displayPizzas = Math.ceil(screen.height / s) * 8;
  for (var i = 0; i < displayPizzas; i++) {
